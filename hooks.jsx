...
function useStickyStore() {
  ...
  const scheduleSave = useCallback((next, opts) => {
    if (saveRef.current) clearTimeout(saveRef.current);
    saveRef.current = setTimeout(() => {
      if (window.stickyAPI) {
        window.stickyAPI.save(next, opts).catch(err => console.warn('[save]', err));
      } else {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setStore(next);
      // Mark this initial save as vault-sync-exempt: the load happens
      // before the renderer imports any files the user edited in the
      // vault while the app was closed, so a vault export here could
      // clobber those edits. Subsequent saves (triggered by setKey)
      // sync normally.
      scheduleSave(next, { skipVaultSync: true });
    })();
...
function useUpdateCheck() {
  ...
  const runCheck = useCallback(async (force) => {
    ...
    try {
      const res = await fetch('https://api.github.com/repos/rokokot/DesktopNotesApp/releases/latest', {
  ...
  useEffect(() => {
    ...
    return window.stickyAPI.onMenuAbout(() => setInfo({
      title: 'About',
      message: `Desktop Notes ${window.stickyAPI.appVersion || ''}`.trim(),
      detail: `Source: https://github.com/rokokot/DesktopNotesApp`,
    }));
  }, []);
